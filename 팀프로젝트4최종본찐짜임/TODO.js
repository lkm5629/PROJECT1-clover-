window.onload = function () {
  ...
  */
  ...
  // 3-1-6-1. HTML에 넣기 전에 특수문자를 안전하게 바꾼다.
  ...
  // 3-1-6-2. HTML 속성값으로 넣기 전에 안전하게 바꾼다.
  function escapeAttribute(value) {
    return escapeHtml(value).replace(/\n/g, "&#10;");
  }

  // 3-1-6-3. 툴팁이 정말 필요한지 먼저 판단한다.
  function shouldUseSmartTooltip(visibleText, fullText, options) {
    const opt = options || {};
    const visible = String(visibleText || "");
    const full = String(fullText || visibleText || "");
    const hasEllipsis = visible.includes("..") || visible.includes("...") || visible.includes("…");
    const normalizedVisible = visible.replace(/(\.\.\.|\.\.|…)$/u, "").trim();
    const normalizedFull = full.trim();
    const longThreshold = Number(opt.longThreshold || 0);

    if (hasEllipsis) return true;
    if (normalizedVisible !== normalizedFull) return true;
    if (longThreshold > 0 && normalizedFull.length > longThreshold) return true;
    return false;
  }

  // 3-1-6-4. 커뮤니티와 같은 말줄임 툴팁 마크업을 만든다.
  function createSmartTooltipMarkup(visibleText, fullText, textClassName, options) {
    const visible = escapeHtml(visibleText || "");
    const full = escapeAttribute(fullText || visibleText || "");
    const useTooltip = shouldUseSmartTooltip(visibleText, fullText, options);

    if (!useTooltip) {
      return `<span class="${textClassName}">${visible}</span>`;
    }

    return `<span class="tooltip-target js-smart-tooltip" tabindex="0" data-tooltip-text="${full}"><span class="${textClassName}">${visible}</span></span>`;
  }
  ...
  cancelModalButton.classList.add("hide");
}
...
function setModalEditStyle() {
  ...
  // 5-1-0. 일정DB 숫자형 페이지네이션 버튼을 그린다.
  function renderDatabasePagination(totalPage) {
    if (!paginationInfoText) return;

    paginationInfoText.innerHTML = "";
    const fragment = document.createDocumentFragment();

    const windowSize = 5;
    let startPage = Math.max(1, currentDatabasePage - Math.floor(windowSize / 2));
    let endPage = Math.min(totalPage, startPage + windowSize - 1);
    startPage = Math.max(1, endPage - windowSize + 1);

    for (let page = startPage; page <= endPage; page += 1) {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "page-number-btn" + (page === currentDatabasePage ? " active" : "");
      button.dataset.page = String(page);
      button.textContent = String(page);
      button.setAttribute("aria-label", `${page}페이지 이동`);
      if (page === currentDatabasePage) {
        button.setAttribute("aria-current", "page");
      }
      fragment.appendChild(button);
    }

    paginationInfoText.appendChild(fragment);
  }
  ...
  function renderDb() {
    ...
    renderDatabasePagination(totalPage);
    ...
    for (const item of pageItems) {
      ...
      tdTitle.innerHTML = createSmartTooltipMarkup(item.title, item.title, "cell-ellipsis", { longThreshold: 14 });
      ...
      tdDetail.innerHTML = createSmartTooltipMarkup(item.detail || "-", item.detail || "상세 내용이 없습니다.", "cell-ellipsis", { longThreshold: 18 });
  ...
  function renderCalendar() {
    ...
    for (let i = 0; i < totalCell; i++) {
      ...
      for (const e of dayEvents) {
        ...
        const chipTooltipText = e.detail ? `${e.title}\n${e.detail}` : e.title;

        chip.innerHTML =
          `<span class="calendar-status">[${st}]</span>` +
          createSmartTooltipMarkup(title, chipTooltipText, "calendar-title", { longThreshold: 18 });
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
        ...
        for (const item of dayEvents) {
          ...
          const weeklyTooltipText = item.detail
            ? `${item.title}\n${item.detail}`
            : item.title;

          eventDiv.innerHTML = `<div class="weekly-event-status">[${st}]</div>
             ${createSmartTooltipMarkup(title, weeklyTooltipText, "weekly-event-title", { longThreshold: 20 })}
  ...
  paginationInfoText.addEventListener("click", function (evt) {
    const button = evt.target.closest(".page-number-btn");
    if (!button) return;
    const nextPage = Number(button.dataset.page || "1");
    if (!Number.isFinite(nextPage) || nextPage < 1) return;
    currentDatabasePage = nextPage;
    renderDb();
  });
...
openTodoModalButton.addEventListener("click", function () {
  ...
  // 8-3-2-1. body 최상단 툴팁 레이어를 만든다.
  const smartTooltipLayer = document.createElement("div");
  smartTooltipLayer.className = "smart-tooltip-layer";
  document.body.appendChild(smartTooltipLayer);

  let activeTooltipTarget = null;

  // 8-3-2-2. 현재 요소 기준으로 툴팁 위치를 계산한다.
  function positionSmartTooltip(target) {
    if (!target) return;

    const rect = target.getBoundingClientRect();
    const gap = 10;
    const layerRect = smartTooltipLayer.getBoundingClientRect();
    ...
    if (left + layerRect.width > viewportWidth - 12) {
      left = viewportWidth - layerRect.width - 12;
    }
    if (left < 12) left = 12;

    if (top + layerRect.height > viewportHeight - 12) {
      top = rect.top - layerRect.height - gap;
    }
    if (top < 12) top = 12;

    smartTooltipLayer.style.left = `${Math.round(left)}px`;
    smartTooltipLayer.style.top = `${Math.round(top)}px`;
  }
  ...
  function showSmartTooltip(target) {
    ...
    smartTooltipLayer.textContent = text;
    smartTooltipLayer.classList.add("is-visible");
    positionSmartTooltip(target);
  }

  // 8-3-2-4. 커스텀 툴팁을 닫는다.
  function hideSmartTooltip() {
    activeTooltipTarget = null;
    smartTooltipLayer.classList.remove("is-visible");
  }

  // 8-3-2-5. 마우스/포커스 이벤트를 위임해서 툴팁을 제어한다.
  document.addEventListener("mouseover", function (evt) {
    const target = evt.target.closest(".js-smart-tooltip");
    if (!target) return;
    showSmartTooltip(target);
  });
  ...
  window.addEventListener("scroll", function () {
    if (activeTooltipTarget) positionSmartTooltip(activeTooltipTarget);
  }, true);
