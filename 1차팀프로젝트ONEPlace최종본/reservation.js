...
function getCapacityFromCard(card) {//최대 수용 인원 20명
...
const roomRules = {
    ...
    // 랩실
    "801호": { allowedDays: [1, 5] },     // 평일
    "802호": { allowedDays: [1, 2, 3, 4, 5] },
    "803호": { allowedDays: [1, 2, 3, 4, 5] },
    "804호": { allowedDays: [1, 2, 3, 4, 5] },
    "805호": { allowedDays: [1, 4, 5] },
    "806호": { allowedDays: [1, 2, 3, 4, 5] },
    "807호": { allowedDays: [1, 2, 3, 4, 5] },
    "808호": { allowedDays: [1, 2, 3, 4, 5] },
    "809호": { allowedDays: [1] },
    "810호": { allowedDays: [1, 2, 3, 4, 5] },
    "811호": { allowedDays: [1, 2, 3, 4, 5] }
};
...
plusBtn.addEventListener('click', () => {
    const current = parseInt(personCount.textContent, 10);
    const next = current + 1;

    if (next > 20) {
        peopleWarningEl.style.display = "block";
        peopleWarningEl.textContent = "최대 20명까지 선택할 수 있습니다.";
        return;
    }

    personCount.textContent = next;
    updatePersonUI();

    peopleWarningEl.style.display = "none";
    peopleWarningEl.textContent = "";
    ...
    reserveBtns.forEach(function (btn) {//버튼마다 클릭 이벤트 등록
        btn.addEventListener('click', function () {//예약하기 버튼 클릭시
            ...
            if (classItem && classItem.dataset.allowed === "0") {
                alert("현재 선택한 날짜는 예약 가능한 호실이 없습니다.");//경고창
                return;
            }
            ...
            const imgSrc = classItem.querySelector("img").getAttribute("src");
            ...
            const center = "클로버 허브 교육센터";

            const duration = e - s; // 예약 시간(예: 10~13 => 3시간)
