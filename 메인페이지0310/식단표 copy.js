  const daysLabel = ["월요일", "화요일", "수요일", "목요일", "금요일", "토요일", "일요일"];
        ...
        function handleVeganClick() {
            if (lastVeganState) { document.getElementById('veganNone').checked = true; lastVeganState = false; }
            else { lastVeganState = true; }
            resetFocus(); updateUI();
        }

        function clearAllergies() {
            document.querySelectorAll('input[name="allergy"]').forEach(cb => cb.checked = false);
            document.getElementById('noAllergy').checked = false;
            updateUI();
        }
        ...
        function renderMeals() {
            ...
            const isVeganFilter = document.querySelector('input[name="vegan"]:checked')?.value === "비건";
            ...
            grid.innerHTML = daysLabel.map((day, index) => {
                ...
                return `<div class="day-card ${highlightClass} ${activeDayIndex === index ? 'active-focus' : ''}" onclick="focusDay(${index}, event)"><div class="card-header">${day}</div>${mealContent}</div>`;
            }).join('');
        }

        function toggleCalendar() { document.getElementById('calendarModal').classList.toggle('active'); if (document.getElementById('calendarModal').classList.contains('active')) renderCalendar(); }
        ...
        function renderCalendar() {
            ...
            for (let d = 1; d <= lastDate; d++) { grid.innerHTML += `<div class="cal-item" onclick="selectDate(${year}, ${month}, ${d})">${d}</div>`; }
        }
        function selectDate(y, m, d) { currentSelectedDate = new Date(y, m, d); viewDate = new Date(y, m, d); activeDayIndex = null; updateUI(); toggleCalendar(); }
