function initAll(){
    ...
    const userAl = loginUser.myAllergies || [];
    ...
    allergies.forEach(e=>{
        // console.log("e", e);
        if(userAl.includes(e)){
            createItem(e, targetList, '제거');
        }else{
            createItem(e, sourceList, '추가');
        }
    })
...
function saveSelection() {
    ...
    //만든 함수 사용
    saveJs("userAllergies", selectedItems);
    loginUser.myAllergies = selectedItems;
...
document.addEventListener('click', function(e) {
    // '선택 완료' 버튼 클릭 시 동작
    if (e.target && e.target.getAttribute('onclick') === 'saveSelection()') {
        setTimeout(() => {
            const savedData = JSON.parse(localStorage.getItem('userAllergies'));
            const loggedInEmail = localStorage.getItem('loggedInUserEmail'); 
            let userList = JSON.parse(localStorage.getItem('UserList')) || [];

            if (savedData && loggedInEmail) {
                const userIndex = userList.findIndex(user => user.email === loggedInEmail);
                if (userIndex !== -1) {
                    // 로그인한 유저 정보에 알레르기 리스트를 저장함
                    userList[userIndex].myAllergies = savedData; 
